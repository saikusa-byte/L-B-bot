    ...
    for event in data.get('events', []):
        ...
        if source_type in ('group', 'room') and group_id:
            ...
            if is_structured_p1 or is_freeform_p1:
                ...
                # 先に返信（タイムアウト防止）→ その後シート書き込み
                p1_prompt = (
                    f"あなたはエリザベスです。株式会社L&Bの専属AIアシスタント秘書です。\n"
                    f"{name}さんがP1報告（トラブル・問題共有）をグループに投稿しました。\n"
                    f"以下の3つの視点から、毎回必ず違う言葉で称えてください：\n"
                    f"①問題を隠さず勇気を持って共有した勇気\n"
                    f"②リスクや異変を早期に察知できる予測危機能力\n"
                    f"③誠実に向き合い改善しようとする姿勢\n"
                    f"2文で。末尾に「記録しました。」と添えてください。"
                )
                fallbacks = [
                    f"📋 {name}さん、勇気ある共有をありがとうございます。誠実に問題と向き合う姿勢が、L&Bをより強くします。記録しました。",
                    f"📋 {name}さん、いち早くリスクを察知して報告してくれた危機予測の力に感謝します。この誠実さがチームの宝です。記録しました。",
                    f"📋 {name}さん、正直に共有してくれてありがとうございます。声を上げる勇気が、会社全体の改善を生みます。記録しました。",
                ]
                p1_msg = gemini_generate(p1_prompt)
                if p1_msg:
                    reply_text = f"📋 {p1_msg}"
                else:
                    now = datetime.now(JST)
                    reply_text = fallbacks[(now.day + now.hour) % len(fallbacks)]
