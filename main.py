...
SHEET_HEADERS = ['日付', '名前', '出社時刻', '退社時刻', '勤務時間', '残業時間(分)', '体調(朝)', '本日タスク', '体調(夜)', '完了タスク', '共有事項']
...
def write_morning_to_sheet(date, name, check_in_time, health_score, tasks):
    try:
        ...
        if row_idx:
            ...
            sheet.update_cell(row_idx, 7, str(health_score))
            sheet.update_cell(row_idx, 8, task_str)
        else:
...
def write_evening_to_sheet(date, name, checkout_time, work_hours, health_score, completed_tasks, shared):
    try:
        ...
        overtime = calc_overtime_minutes(checkout_time)
        overtime_str = f"{overtime}分" if overtime > 0 else "0分"
        if row_idx:
            sheet.update_cell(row_idx, 4, checkout_time)
            sheet.update_cell(row_idx, 5, work_hours)
            sheet.update_cell(row_idx, 6, overtime_str)
            sheet.update_cell(row_idx, 9, str(health_score))
            sheet.update_cell(row_idx, 10, tasks_str)
            sheet.update_cell(row_idx, 11, shared)
        else:
            sheet.append_row([date, name, '', checkout_time, work_hours, overtime_str, '', str(health_score), tasks_str, shared, ''])
    except Exception as e:
...
def update_monthly_summary(year_month=None):
    ...
    try:
        ...
        # 対象月のデータを集計
        # 列: 日付,名前,出社時刻,退社時刻,勤務時間,残業時間(分),体調(朝),本日タスク,体調(夜),完了タスク,共有事項
        summary = {}
        for row in all_rows[1:]:
            ...
            if name not in summary:
                summary[name] = {'days': 0, 'total_minutes': 0, 'overtime_minutes': 0, 'am_scores': [], 'pm_scores': []}
            ...
            if len(row) > 5 and row[5]:
                ot = re.search(r'(\d+)分', row[5])
                if ot:
                    summary[name]['overtime_minutes'] += int(ot.group(1))

            # 体調点数(朝)
            try:
                if len(row) > 6 and row[6]:
                    summary[name]['am_scores'].append(float(row[6]))
            except Exception:
                pass
            # 体調点数(夜)
            try:
                if len(row) > 8 and row[8]:
                    summary[name]['pm_scores'].append(float(row[8]))
            except Exception:
                pass
        ...
        # ヘッダー設定
        headers = ['年月', '名前', '出社日数', '合計勤務時間', '残業時間合計(分)', '平均体調(朝)', '平均体調(夜)']
        ...
        for name, data in summary.items():
            ...
            new_row = [year_month, name, data['days'], total_str, data['overtime_minutes'], am_avg, pm_avg]

            # 既存行を探す
            found = False
            for i, row in enumerate(existing[1:], 2):
                if len(row) >= 2 and row[0] == year_month and row[1] == name:
                    monthly_sheet.update(f'A{i}:G{i}', [new_row])
                    found = True
                    break
            if not found:
...
def parse_p1_report(text):
    ...
    result = {
        'date':     extract(r'①日時\s*[：:]\s*(.+?)' + stop, text),
        'project':  extract(r'②案件名\s*[：:]\s*(.+?)' + stop, text),
        'summary':  extract(r'③概要\s*[：:]\s*(.+?)' + stop, text),
        'content':  extract(r'④内容\s*[：:]\s*(.*?)' + stop, text),
        'cause':    extract(r'⑤原因\s*[：:]\s*(.*?)' + stop, text),
        'client':   extract(r'⑥クライアント対応策\s*[：:]\s*(.*?)' + stop, text),
        'partners': extract(r'⑦関係者対応策\s*[：:]\s*(.*?)' + stop, text),
